...
function RevealTab() {
  ...
  const handleRevealSecret = async () => {
    if (!key.trim()) {
      setError('Introduce tu clave secreta');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Error al revelar mensaje');
      }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {loading ? 'Mostrando...' : 'Revelar mensaje'}
      </button>
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">
          ...
          <p className="text-xs mt-2 text-red-600">
            Clave inexistente o ya fue usada
          </p>
        </div>
      )}
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 space-y-3">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded p-3">
            <p className="text-yellow-800 text-sm">
               <strong>Atención:</strong> El mensaje se borrará y no puede ser visto de nuevo
            </p>
          </div>
        </div>
      )}
